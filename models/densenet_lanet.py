...
class DenseNet(nn.Module):
    ...
    def __init__(
        ...
        self.bn_clf = nn.BatchNorm2d(1024)
        self.fc_clf = nn.Linear(1024, 2)
        self.reduce_dim = nn.Conv2d(2*1024, 1024,kernel_size=1,bias=False) # Way2
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        for block in self.features[:-3]:
            x=block(x)
            # print(x.shape)
        
        feat = x
        feat = self.bn_clf(feat)
