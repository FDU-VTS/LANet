...
class Inception3(nn.Module):
    def __init__(
        ...
        self.fc_clf = nn.Linear(768, 2)
        self.reduce_dim = nn.Conv2d(768*2, 768,kernel_size=1,bias=False)
        if adaloss:
    ...
    def forward(self, x: Tensor) -> InceptionOutputs:
        ...
        out_clf, x, aux = self._forward(x)
        aux_defined = self.training and self.aux_logits
        if torch.jit.is_scripting():
            if not aux_defined:
                warnings.warn("Scripted Inception3 always returns Inception3 Tuple")
            return out_clf, InceptionOutputs(x, aux)
        else:
            return out_clf, self.eager_outputs(x, aux)
