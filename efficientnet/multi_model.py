...
class EfficientNet(nn.Module):
    ...
    def __init__(self, blocks_args=None, global_params=None):
        ...
        if self._global_params.include_top:
            ...
            self.fc_clf = nn.Linear(round_filters(112, self._global_params), 2)
        ...
        self.reduce_dim = nn.Conv2d(round_filters(112, self._global_params)*2, round_filters(112, self._global_params),kernel_size=1,bias=False)
    ...
    def extract_features(self, inputs):
        ...
        for idx, block in enumerate(self._blocks):
            ## TODO
            ## only for efficient-b6
            if idx == len(self._blocks) - block_id:
                break
            drop_connect_rate = self._global_params.drop_connect_rate
            if drop_connect_rate:
                drop_connect_rate *= float(idx) / len(self._blocks)  # scale drop connect_rate
    ...
    def forward(self, inputs):
        ...
        """
        # Convolution layers
        # x, out_clf, cam = self.extract_features(inputs)
        x, out_clf = self.extract_features(inputs)
